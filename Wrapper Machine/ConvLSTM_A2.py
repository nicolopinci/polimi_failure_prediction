...
os.environ["CUDA_VISIBLE_DEVICES"] = "1"  # Replace "1" with the index of the second GPU you want to use (0 for the first GPU, 1 for the second, and so on)
...
from tensorflow.keras.layers import Dropout, BatchNormalization, Dense, ConvLSTM1D
import imblearn
import datetime
import gc
...
def build_model_LSTM(input_shape):
    model = Sequential()
    model.add(tf.keras.layers.Reshape((input_shape[0], 1, input_shape[1]), input_shape=input_shape))
    model.add(ConvLSTM1D(filters=16, kernel_size=1, return_sequences=True))
    model.add(Flatten())
...
## GLOBAL VARIABLES
EXPERIMENT_ID = f'PRED_{HOURS}h_READ_{SLIDING_WINDOW_LEN}steps_FREQ_{SAMPLING_FREQ}s_ConvLSTM_a2'
