...
os.environ["CUDA_VISIBLE_DEVICES"] = "0"  # Replace "1" with the index of the second GPU you want to use (0 for the first GPU, 1 for the second, and so on)
...
def build_model_LSTM(input_shape):
    ...
    model.add(ConvLSTM1D(filters=16, kernel_size=16, return_sequences=True, padding = "same", data_format='channels_last'))
    model.add(Flatten())
...
## GLOBAL VARIABLES
EXPERIMENT_ID = f'PRED_{HOURS}h_READ_{SLIDING_WINDOW_LEN}steps_FREQ_{SAMPLING_FREQ}s_ConvLSTM_a4'
