...
BATCH_SIZE = 128 # 256 for < 35 minutes
...
def build_model_Transformer(input_shape, head_size=16, num_heads=8, ff_dim=2, num_transformer_blocks=2, mlp_units=[8, 4], dropout=0.4, mlp_dropout=0.5):
...
## GLOBAL VARIABLES
EXPERIMENT_ID = f'PRED_{HOURS}h_READ_{SLIDING_WINDOW_LEN}steps_FREQ_{SAMPLING_FREQ}s_Transformer_a4'
...
for model_name in models:
    ...
    for train, test in kfold.split(X, y):
        ...
        for RUS_i in range(10):
            ...
            early_stopping = tf.keras.callbacks.EarlyStopping(monitor='val_loss', min_delta=0, patience = 5, verbose=0, mode='min', restore_best_weights=False)
            ...
            if model_name == 'Transformer':
                model = build_model_Transformer((SLIDING_WINDOW_LEN, X_train_res_scale.shape[2]))
            ...
            float_pred_test = model.predict(X_test_res_scale, verbose=0)
            pred_test = np.rint(float_pred_test)
