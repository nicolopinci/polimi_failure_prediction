...
os.environ["CUDA_VISIBLE_DEVICES"] = "1"  # Set the second GPU as the visible device
...
def build_model_LSTM(input_shape):
    ...
    model.add(tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(128, return_sequences=True, kernel_regularizer=tf.keras.regularizers.L2(0.001)), input_shape=input_shape))
    model.add(tf.keras.layers.BatchNormalization())
    model.add(tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(64, return_sequences=True, kernel_regularizer=tf.keras.regularizers.L2(0.001))))
    model.add(tf.keras.layers.BatchNormalization())
    model.add(tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(128, return_sequences=False, kernel_regularizer=tf.keras.regularizers.L2(0.001))))
    model.add(tf.keras.layers.BatchNormalization())
...
parser.add_argument('--pred_hours', type=float, default=4.0)
parser.add_argument('--read_minutes', type=int, default=10)
...
## GLOBAL VARIABLES
EXPERIMENT_ID = f'PRED_{HOURS}h_READ_{SLIDING_WINDOW_LEN}steps_FREQ_{SAMPLING_FREQ}s_LSTM_BCEBidirectional'
        ...
        for RUS_i in range(10):
            ...
            try:
                ...
                if model_name == 'LSTM':
                    ...
                    model = tf.keras.models.load_model(f'{PATH}{EXPERIMENT_ID}/best_model_tmp.h5')
