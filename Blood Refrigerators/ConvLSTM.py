...
class ConvLSTMHyperModel(HyperModel):
    ...
    def build(self, hp):
        ...
        model.add(tf.keras.layers.ConvLSTM1D(filters=hp.Int('filters', min_value=8, max_value=64, step=8),
                                             return_sequences=True,
                                             data_format='channels_last',
                                             kernel_size=hp.Int('kernel_size', min_value=1, max_value=7, step=2)))
...
hypermodel = ConvLSTMHyperModel(input_shape)
...
df.to_csv("ConvLSTM_" + folder + "_RW_" + str(RW_minutes) + "min.csv", index=False)
